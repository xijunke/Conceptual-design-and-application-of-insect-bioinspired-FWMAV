...
F_yadd=0.2*F_normal((locat_max:num),2);
...
xlswrite('D:\KXJ\PassiveRot_dynamic_Science_fruitfly\wing_parameter\datanalysis_science_fruitfly\wing_model\Stick_figure\wingmotion_oneT_simpres.xlsx',A,'sheet1','A1:G2000');
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% % Nframe = np*7.19828;   % number of movie frames -------------------------- for change
% % Nframe = np*14.3966;
...
% if animation == 1
%     aviobj = avifile('ElmentMotion_2D.avi','fps',Nspeed); % filename --- for change
% end
%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Nskip=50;
% Nstep=2000;
...
Nskip=8;
Nstep=334;
% L=[1:Nskip:Nstep]; % length=size(L);  % (1*42)
%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
for i = 1:Nskip:Nstep   % L=[1:Nskip:Nstep]; % size(L)=1*40;
    ...
    R_w2s = Yaw*Pitch;     % from wing frame to spar frame——从翅坐标系到体坐标系
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % % 前缘——实心圆点绘制
    x1=R_eff;
    y1=0;
...
plot(p_lead(2,1),p_lead(3,1),'ro-','LineWidth',1.5,'MarkerEdgeColor','r','MarkerFaceColor','r','MarkerSize',6)
hold on
plot([p_lead(2,1),p_tail(2,1)],[p_lead(3,1),p_tail(3,1)],'r-','LineWidth',1.5) % 绘制前缘后缘实线
hold on
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
Fytran_cop= R_w2s*F_ytran_cop;        % wing_chord in body frame  % (3*2)
...
quiver(Fytran_cop(2,1),Fytran_cop(3,1),...                                                               % 起点
             Fytran_cop(2,2),Fytran_cop(3,2),0.5,'b-','LineWidth',2.0); % 比例因子0.5      % 终点
hold on
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% (b) 中弦点—绘制力失
x_mid=[x4,0];
...
Fyadd_mid= R_w2s*F_yadd_mid;        % wing_chord in body frame  % (3*2)
...
quiver(Fyadd_mid(2,1),Fyadd_mid(3,1),...                                                               % 起点
             Fyadd_mid(2,2),Fyadd_mid(3,2),0.5,'g-','LineWidth',2.0); % 比例因子0.5      % 终点
hold on
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%
% 动画片段程序结束段
