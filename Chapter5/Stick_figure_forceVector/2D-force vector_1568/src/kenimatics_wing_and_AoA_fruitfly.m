...
% % 翅运动规律和几何攻角(AOA)
% % (注意：最终计算出来的扭转角,需要时间轴区间和实验时间轴配准)———需要求解和与核对
% % clear all;clc;
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% %% 创建符号函数，求导——获得角速率
% % (1) 2014-Science-实测翅扭转角—— 按照4阶傅里叶级数拟合 
% syms t         % t  自己设置时间轴
% w =1185.6;     % 角频率   %  f=188.7; T=1/f;  %翅拍频率 (Hz)和周期  
% phi1=sym('(3.9008+65.0445*cos(w*t)+4.2642*sin(w*t)+3.5806*cos(2*w*t)-2.9492*sin(2*w*t)+0.1319*cos(3*w*t)+0.3639*sin(3*w*t)+0.7844*cos(4*w*t)+0.2098*sin(4*w*t))*pi/180');  %符号函数
% dphi1=diff(phi1,t,1);
...
% psi1=sym('(4.2936+1.8536*cos(t*w)+59.6529*sin(t*w)+5.1852*cos(2*t*w)+1.6095*sin(2*t*w)-8.4569*cos(3*t*w)+9.8057*sin(3*t*w)+3.9562*cos(4*t*w)+5.8064*sin(4*t*w)-3.0337*cos(5*t*w)-2.8749*sin(5*t*w)-2.8771*cos(6*t*w)+0.6686*sin(6*t*w)+0.8649*cos(7*t*w)-0.6137*sin(7*t*w)+0.0771*cos(8*t*w)-1.0007*sin(8*t*w))*pi/180');  %符号函数
% dpsi1=diff(psi1,t,1);   
...
% % t=linspace(0.0028,0.0028+2*T,200);  % t_steady1                                                                %输出
% t=linspace(0.0028,0.0028+T,200);  % t_steady1  
% dphi=dphi(w,t);               %(1*200)的行向量——拍打角速率                                                     %输出
% ddphi=ddphi(w,t);          %(1*200)的行向量——拍打角加速率                                                  %输出
% dpsi=dpsi(w,t);          %(1*200)的行向量——扭转角速率                                                           %输出
% ddpsi=ddpsi(w,t);     %(1*200)的行向量——扭转角加速率                                                        %输出
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% %% 拍打角和扭转角
% % 拍打角—— (1*200)的行向量——拍打角——弧度制            %输出                                     
...
% figure(1)                                                          % 图1——拍打角和扭转角
% plot(t/T,phi*180/pi,'r-',t/T,psi*180/pi,'b-')  %转换为ms 和 度数degree   *10^3   *180/pi
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% (3) 将截取的单个稳定的周期——以攻角变化周期为区间的运动学数据读入——弧度制
wingmotion_oneT=xlsread('wingmotion_oneT.xlsx','A1:G2000');
% f=188.7;  %T=1/f;  %翅拍频率 (Hz)和周期  % w =1185.6; 
t=wingmotion_oneT(:,1);             % 单位是ms
psi=wingmotion_oneT(:,2);            % 扭转角——单位是rad
dpsi=wingmotion_oneT(:,3);            % 扭转角——单位是rad
ddpsi=wingmotion_oneT(:,4);             % 扭转角——单位是rad
phi=wingmotion_oneT(:,5);                   % 拍打角——单位是rad
dphi=wingmotion_oneT(:,6);                     % 拍打角——单位是rad
ddphi=wingmotion_oneT(:,7);                      % 拍打角——单位是rad
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Plot-扭转角和几何攻角AOA
alpha1=pi/2+psi.*sign(dphi);        %(1*200)的行向量——几何攻角——弧度制        %输出——全正几何攻角 
dalpha1=dpsi.*sign(dphi);             %(1*200)的行向量——几何攻角-角速率             %输出——有正有负
ddalpha1=ddpsi.*sign(dphi);         %(1*200)的行向量——几何攻角-角速率             %输出——有正有负
...
alpha2=atan2(omega_z,-omega_y);      % 这里输出: 正负几何攻角—不提倡使用XXX几何攻角—但是取绝对值是正确
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Plot-拍打角速率和扭转角速率
%% 拍打角速率dphi和扭转攻角角速率dalpha见前文描述
% figure(4)                                                              % 图4——拍打角速率和扭转角速率
% % plot(t/T,abs(dphi),'r-',t/T,abs(dalpha1),'b-')           % 拍打角速率和扭转角速率随时间的变化规律
...
% grid on
%% Plot-拍打角加速率，扭转角加速率，攻角角加速率
%拍打角加速率ddphi和扭转攻角角加速率ddalpha1见前文描述
% figure(5)        % 图5—拍打角加速率, 扭转角加速率, 攻角角加速率随时间的变化规律
...
% title('拍打角加速率，扭转角加速率，攻角角加速率随时间的变化规律') 
% grid on
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
C_L =sign(alpha).*(0.225+1.58*sin((2.13*abs(alpha)*180/pi-7.2)*pi/180));  %角度alpha以度数表示,但是三角函数是针对弧度制的哦
C_D =sign(alpha).*(1.92-1.55*cos((2.04*abs(alpha)*180/pi-9.82)*pi/180));  %角度alpha以度数表示,但是三角函数是针对弧度制的哦—1999-Science-MH Dickinson
% C_D =1.92+1.55*cos((2.04*alpha1-9.82)*pi/180);      %角度alpha以度数表示,但是三角函数是针对弧度制的哦—2004-JEB-Wang ZJ
...
% % C_LD=plot(t/T,C_L,'r-',t/T,C_D,'b-');      %时间正则化，乘以频率f, 或者除以周期T;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% %三种法向气动力系数
% figure(8)     % 图8—翅坐标系下—法向气动力系数
% C_N=plot(t/T,C_N1,'r-',t/T,C_N2,'b-',t/T,C_N3,'g-');      %时间正则化，乘以频率f, 或者除以周期T;
% xlabel('\itt (Normalized time with flapping period)')
% ylabel('\itForce coefficients')
