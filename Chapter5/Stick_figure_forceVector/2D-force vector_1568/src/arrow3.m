...
%   ARROW3(P1,P2,S,W,H,IP,ALPHA,BETA) can be used to specify properties of the line, initial point marker, and arrowhead.  

%   (S W H IP AlPHA BETA)
%   S is a character string made with one element from any or all of the following 3 columns:
...
%   lighter shade, e.g. '^q' is light kumquat.  
% 
%   The relative brightness of light and dark color shades is controlled by the scalar parameter BETA.  
%    Color code prefixes do not affect black (k), white (w), or the special color switches (xo|).
%
...
%   %(S W H IP AlPHA BETA)
%   W (default = 1) is a vector of arrowhead widths; use W = 0 for no arrowheads. %箭头宽度 
%   H (default = 3W) is a vector of arrowhead heights.   % 箭头高度

%   %(S W H IP AlPHA BETA)
%   If  vector IP is neither empty nor negative, initial point markers will
