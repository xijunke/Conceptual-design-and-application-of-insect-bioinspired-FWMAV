...
title('压电驱动器的尖端位移输出')
%%%%%%%%%%%%%%%%%%%%%%%%%
% The compressive strain in each of the two PZT plates
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% The time-varying electric field applied to each of the PZT plates
...
%%%%%%%%%%%%%%%%%%%%%%%%%
% The effective mass felt by the actuator
M_e=(I_zz+I_am)*T^2;
% The stiffness felt by the actuator tip
K=K_act+T^2*k_t;
%% The driving force provided by the actuator
...
f_31min=14;   % N/Vm
f_31max=29;  % N/Vm
a_2=-230;       % a slight linear increase in the force with increasing compressive strain
% a_3=10e-5; % ——2015-SMS
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% 
rou=1.225e-3;                  % density of air %Kg/m^3=10^6/(10^3)^3=10^(-3)mg/mm^3
A_w=54.59e-6;                 % mm^2  % wing area
R_cp=(1.42*9.56)*10^-3; % mm % radial center of pressure
r_nd2=0.564;                     % standard wing-shape parameter
R=17e-3;                          % mm % distance from the transmission flexure to the wing tip
W_av=A_w/R;                  % mean width of the wing
w_nomd2=0.43;     
W=4.5e-3; % mm   
W_cp1=(0.39*1.93)*10^-3;  % mm 
p_min=0.132;         % fitting parameters
p_max=0.151;       % fitting parameters
k_wh=(0.985*1.52)*10^-6;  % uNm/rad=*10^-6Nm/rad  % the stiffness of the passive wing hinge
b_min=0.39*0.4;                % the drag at  psi= 0 degree.
b_max=1.25*3.4;               % the drag at  psi=90 degree
b_1=(b_max+b_min)/2;
b_2=(b_max-b_min)/2;
b_L=1.8;                  
%% 
