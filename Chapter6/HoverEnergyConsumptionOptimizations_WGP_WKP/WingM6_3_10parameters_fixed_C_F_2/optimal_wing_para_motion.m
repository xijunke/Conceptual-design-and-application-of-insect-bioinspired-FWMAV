...
% options=gaoptimset('Display','iter','PopulationSize',100,'Generations',350,...   % 回复继续搜索，将代数=200修改为代数=5+10
...
%     'TolCon',1e-15,'TolFun',1e-15,'UseParallel','always'); %并行计算——可行
%%%%%%%%%%%%%%%%%%%%%%%%%%%
% x_start=[31.0614,1.5655,0.0339,1.0516,9.2061,1.9760,3.9880,1.9642,1.9748,0.0145]; % 继续遗传5代+2000代局部搜索的结果
% x_start=[49.0006,1.4463,0.0759,0.5151,0.3459,-1.1952,3.8347,1.9398,1.9360,0.0104];% 继续遗传5代+2000代局部搜索的结果
% x_start=[50.5894,1.5061,0.3189,0.5494,0.8681,-1.3308,3.4759,1.7892,1.9606,0.0113];% 继续遗传50代+2000代局部搜索的结果
% x_start=[5.0677789e+001,1.1762423e+0,5.1307852e-001,8.1914557e-001,2.5339154e+0,-1.4619157e+0,...
%               3.6447180e+0,1.8919010e+0,1.9353045e+0,1.2250774e-007];% 继续遗传10代+2000代局部搜索的结果
% x_start=[5.0250945e+001,1.1885097e+0,5.6727703e-001,8.8076414e-001, 2.6712153e+0,-1.5707963e+0,...
% 3.6218464e+0,1.8655888e+0,1.9471743e+0,-7.0895123e-007]; % 继续遗传20代+2000代局部搜索的结果
% x_start=[188.0,1.125,0.01,1.2201,5,-1.5427,3.004,0.8854,0.3289,0.356737]; % 继续遗传50代+2000代局部搜索的结果
x_start=[46.545,1.30898,0.0154,1.2716,2.5,-1.5167,3.9809,1.964,1.7827,0.0000];  % P_asterisk =4.7922; L =1.0020;
options=gaoptimset('Display','iter','PopulationSize',100,'InitialPopulation',x_start,'Generations',50,...   % 回复继续遗传5代
    'PlotFcns',{@gaplotbestf,@gaplotexpectation,@gaplotstopping,@gaplotbestindiv,@gaplotscores,@gaplotrange},...
    'TolCon',1e-15,'TolFun',1e-15,'UseParallel','always'); %并行计算——可行
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% fminsearchoptions =
...
% fminsearchoptions=optimset('Display','iter-detailed',...
%                                  'MaxFunEvals',3000,'MaxIter',3000,'TolX',1e-15,'TolFun',1e-15,'UseParallel','always');% 初始搜索4
% fminsearchoptions=optimset('Display','iter-detailed',...
%                                  'MaxFunEvals',2000,'MaxIter',2000,'TolX',1e-15,'TolFun',1e-15,'UseParallel','always');% 初始搜索5
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
fminsearchoptions=optimset('Display','iter-detailed', 'MaxFunEvals',2000,'MaxIter',2000,'TolX',1e-15,'TolFun',1e-15,'UseParallel','always');% 初始搜索5
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
C_aver=x(8);    % mm
xr0=x(9);
R_wing1=x(7)*10^-3;  %   R_wing=3.004*10^-3;            % m
C_aver1=x(8)*10^-3;   %  C_aver=0.8854*10^-3;            % m
xr01=x(9)*10^-3; 
f=x(1);               % 
% f=188.7;                  % Hz
phi_aver=x(2);   % 
